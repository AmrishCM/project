    ...
    if request.method == 'POST':
        ...
        if validVoter:
            if not registered:
                ...
                # Generate OTP and print to terminal using sms() from xtra.py
                otp_number = sms(voter.ph_country_code + voter.phone_number)
...
def otp(request):
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, 'poll/failure.html', {'fail': 'Voter not found'})

        fail = ''
        if int(otp_input) == int(Voter.otp):
...
def signin(request):
    ...
    if time[0].end > now_utc and time[0].start < now_utc:
        if request.method == 'POST':
        ...
        format = "%d/%m/%Y at %H:%M:%S %Z%z"
        if time[0].end < now_utc:
            asia = time[0].end.astimezone(timezone('Asia/Kolkata'))
            context = {'fail': "Voting ended on " + asia.strftime(format)}
        else:
            asia = time[0].start.astimezone(timezone("Asia/Kolkata"))
            context = {'fail': "Voting will start on " + asia.strftime(format)}
        return render(request, 'poll/failure.html', context)


@login_required(login_url='login')
def create(request, pk):
    voter = models.Voter.objects.get(username=request.user.username)
    if request.method == 'POST' and request.user.is_authenticated and not voter.has_voted:
        vote = pk
        lenVoteList = models.Vote.objects.count()
        block_id = math.floor(lenVoteList / 5) + 1 if lenVoteList > 0 else 1
        ...
        try:
            privateKey_d, privateKey_n = decrypt(phrase,
                                                voterpvt[0]['private_key_d'],
                                                voterpvt[0]['private_key_n'],
                                                voterpvt[0]['salt'])
        except:
            logout(request)
...
def seal(request):
    if request.method == 'POST':
        ...
        if models.Vote.objects.count() % 5 != 0:
            logout(request)
            return render(request, 'poll/votesuccess.html', {'code': vote_id})
        else:
            transactions = list(models.Vote.objects.order_by('block_id').reverse()[:5])
            ...
            vote_auth.save()
            block = models.Block(id=block_id, prev_hash=prev_hash, self_hash=self_hash,
                                 merkle_hash=merkle_hash, nonce=nonce, timestamp=timestamp)
            block.save()
    ...
    if time[0].end < now_utc:
        if request.method == 'GET':
            ...
            if tampered_block_list:
                verification = 'Verification Failed. Following blocks have been tampered --> {}. The authority will resolve the issue'.format(
                    tampered_block_list)
                error = True
            else:
                verification = 'Verification successful. All votes are intact!'
                error = False
                votes = [retDate(x) for x in models.Vote.objects.order_by('timestamp')]
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if tampered_block_list:
                    ...
                    error = False
                    vote = [retDate(x) for x in models.Vote.objects.filter(id=unique_id)]
            except:
                vote = []
                error = True
        ...
        format = "%d/%m/%Y at %H:%M:%S %Z%z"
        asia = time[0].end.astimezone(timezone('Asia/Kolkata'))
        context = {'fail': "Verification will enable on " + asia.strftime(format)}
        return render(request, 'poll/failure.html', context)
...
def result(request):
    ...
    if time[0].end < now_utc:
        if request.method == "GET":
            ...
            if voteVerification:
                return render(request, 'poll/verification.html',
                              {'verification': "Verification failed. Votes have been tampered in following blocks --> {}. The authority will resolve the issue".format(
                                  voteVerification), 'error': True})

            vote_auth = models.VoteAuth.objects.get(username='admin')
            if not vote_auth.resultCalculated:
                vote_auth.resultCalculated = True
                vote_auth.save()
                for vote in models.Vote.objects.all():
                    candidate = models.Candidate.objects.get(candidateID=vote.vote)
                    candidate.count += 1
                    candidate.save()
        ...
        format = "%d/%m/%Y at %H:%M:%S %Z%z"
        asia = time[0].end.astimezone(timezone('Asia/Kolkata'))
        context = {'fail': "Result will be displayed after " + asia.strftime(format)}
        return render(request, 'poll/failure.html', context)
